...
# 																		# virtualhost name added, case sencetive
# declare -a webroots=("/home/\${webmaster}/Web/\${host}" "/var/www/\${host}" "/var/www/\${webmaster}/\${host}")
# declared below, after a chanse to edit included variadles
...
info_user() {
	...
	if in_terminal; then
		...
		zenity --info --text="$msg" --icon-name="${windowicon}" --no-wrap --title="Virtualhost" 2>/dev/null
	fi
}
...
# apt hangs when run from script, thats why terminal needed
install_zenity() {
...
validate_domain() {
  $(LANG=C; validate_input "Bad domain with leading . (dot)" "$1" "^\.[a-zA-Z0-9]([a-zA-Z0-9-]{0,61}[a-zA-Z0-9\.])*$") || exit 1
	exit 0
}

validate_username() {
	$(LANG=C; validate_input "Bad username." "$1" "^[[:lower:]_][[:lower:][:digit:]_-]{2,15}$") || exit 1
...
validate_email() {
	$(LANG=C; validate_input "Bad admin email." "$1" "^[a-z0-9!#\$%&'*+/=?^_\`{|}~-]+(\.[a-z0-9!#$%&'*+/=?^_\`{|}~-]+)*@([a-z0-9]([a-z0-9-]*[a-z0-9])?\.)*[a-z0-9]([a-z0-9-]*[a-z0-9])?\$") 	|| exit 1
	exit 0
}

validate_subdomain() {
	$(LANG=C; validate_input "Bad subdomain" "$1" "^[a-zA-Z0-9]([a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?$") || exit 1
	exit 0
}
    ...
    case "$1" in
        ...
        "-g"|"--gui")
						if [ -z "$DISPLAY" ];then
							...
							gui_set=1
							# GUI can be enabled at this point, when run from terminal with --gui, so check again
							$(check_gui) || exit
            	in_terminal() {
								false
							}
        ...
        "-h"|"--help")
        		cat << EOF
usage: ./${0}		# if some arguments specified in command line, script will not ask to edit it value
       [--webmaster="userlogin"]        # user with permissions to edit www files. group is 'www-data'
       [--webgroup="www-data"]					# group of apache2 service user
       [--domain=".localhost"]          # leading dot required
       [--subdomain="Example"]          # Subdomain and webroot folder name(folder case censetive)
...
EOF
	...
	if [ "$maindomain_set" ]; then
		$(validate_domain "$maindomain") || msg="Bad value for --domain. Should have leading dot.  \".localhost\"\n"
	fi
...
[ -z "$mysqlskip" ] && if have_command mysqld; then
	if [ -z "$mysqlskip" ]; then mysqladminpwd=$(get_text_input "Admin password" "Admin password (${mysqladmin})\nClick cancell or enter empty value to disable mysql conf" "" "skipcheck") || mysqlskip=1; fi
	if [ "$mysqlauto" ]; then
		mysqldb="$host"
		mysqluser="$host"
		mysqlpwd=""
  else
		if [ -z "$mysqlskip" ]; then mysqldb=$(get_text_input "Database" "Database name(Enter for default)" "$host") || mysqlskip=1; fi
		if [ -z "$mysqlskip" ]; then mysqluser=$(get_text_input "Username" "Mysql user for db:$mysqldb host:localhost(Enter for default)" "$mysqldb") || mysqlskip=1; fi
		if [ -z "$mysqlskip" ]; then mysqlpwd=$(get_text_input "Password" "Mysql password for user:$mysqluser db:$mysqldb host:localhost(Enter for empty)" "" "skipcheck") || mysqlskip=1; fi
	fi
...
if [ $(cat "$tmpresult") ]; then notify_user "Virtualhost added. Apache2 reloaded."; fi
